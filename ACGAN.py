...
class ACGAN(object):
    ...
    def __init__(self, sess, epoch, batch_size,
        ...
        if dataset_name == 'mnist' or dataset_name == 'fashion-mnist':
            ...
            self.LAMBDA = 0.05 #scale of gradient penlty
            self.BETA = 10 #scale of classification error
    ...
    def discriminator(self, x, reuse=False):
        with tf.variable_scope('discriminator', values=[x], reuse=reuse): 
            with slim.arg_scope(self.disc_arg_scope()):
                ...
                real_fake = slim.fully_connected(net, 1, 
                                           activation_fn=None,
                                           normalizer_fn=None,
                                           normalizer_params=None,
                                           scope='real_or_fake')
                y_ = slim.fully_connected(net, self.y_dim,
                                          activation_fn=None,
                                          normalizer_fn=None,
                                          normalizer_params=None,
